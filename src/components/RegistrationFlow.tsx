...
export default function RegistrationFlow({ onCompleteRegistration, lang = "fr" }: RegistrationFlowProps) {
  ...
  // Birth details states (Mandatory)
  const [birthDay, setBirthDay] = useState("");
  const [birthMonth, setBirthMonth] = useState("");
  const [birthYear, setBirthYear] = useState("");
  const [birthCountry, setBirthCountry] = useState("République Démocratique du Congo");
  const [birthProvince, setBirthProvince] = useState("Kinshasa");
  ...
  const handleNextStep = () => {
    if (step === 1) {
      ...
      if (!birthDay || !birthMonth || !birthYear) {
        setErrorMsg("La date de naissance complète (Jour, Mois, Année) est obligatoire.");
        return;
      }
      if (!birthCountry.trim()) {
        setErrorMsg("Le pays de naissance est obligatoire.");
        return;
      }
      if (!birthProvince.trim()) {
        setErrorMsg("La province de naissance est obligatoire.");
        return;
      }
      if (!birthCity.trim()) {
        setErrorMsg("La ville ou territoire de naissance est obligatoire.");
        return;
      }
      ...
      <div className="p-6 md:p-8">
        ...
        {step === 3 && (
          <div className="space-y-6">
            ...
            {/* DATE & LIEU DE NAISSANCE OBLIGATOIRE */}
            <div id="birth-details-section" className="border-t border-slate-200 pt-5 space-y-4 text-left">
              <h4 className="text-xs font-black text-slate-700 uppercase tracking-widest flex items-center gap-1.5">
                <span className="text-blue-600">📅</span>
                <span>Date & Lieu de Naissance (Obligatoire)</span>
                <span className="bg-red-100 text-red-700 text-[9px] font-bold px-2 py-0.5 rounded-full uppercase">Souveraineté</span>
              </h4>

              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {/* Date de Naissance (Jour-Mois-Année) */}
                <div className="bg-slate-50 p-4 rounded-xl border border-slate-200 space-y-3">
                  <label className="block text-xs font-bold text-slate-700">Date de naissance <span className="text-red-500">*</span></label>
                  <div className="grid grid-cols-3 gap-2">
                    <div>
                      <select
                        id="birth-day-picker"
                        ...
                        className="w-full bg-white border border-slate-200 text-slate-800 rounded-xl px-2 py-2 text-xs focus:border-blue-600 focus:ring-1 focus:ring-blue-600 outline-none cursor-pointer font-bold"
                      >
                  ...
                  <span className="text-[10px] text-slate-500 mt-1 block">Format officiel Jour-Mois-Année exigé pour la vérification criminelle d'identité.</span>
                </div>

                {/* Lieu de Naissance */}
                <div className="bg-slate-50 p-4 rounded-xl border border-slate-200 space-y-3">
                  <label className="block text-xs font-bold text-slate-700">Lieu de naissance <span className="text-red-500">*</span></label>
                  
                  <div className="space-y-2.5">
                    {/* Pays & Province de naissance */}
                    <div className="grid grid-cols-2 gap-2">
                      <div>
                        <label className="block text-[10px] font-bold text-slate-500 mb-0.5">Pays <span className="text-red-500">*</span></label>
                        <input
                          type="text"
                          ...
                          className="w-full bg-white border border-slate-200 text-slate-800 rounded-xl px-3 py-1.5 text-xs focus:border-blue-600 outline-none font-bold"
                        />
                      </div>
                      <div>
                        <label className="block text-[10px] font-bold text-slate-505 mb-0.5">Province <span className="text-red-500">*</span></label>
                        {birthCountry === "République Démocratique du Congo" || birthCountry === "RDC" || birthCountry === "Congo" ? (
                          <select
                            id="birth-province-picker"
                            value={birthProvince}
                            onChange={(e) => setBirthProvince(e.target.value)}
                            className="w-full bg-white border border-slate-200 text-slate-800 rounded-xl px-3 py-1.5 text-xs focus:border-blue-600 outline-none font-bold cursor-pointer"
                          >
                            <option value="">-- Choisir --</option>
                            {drcProvinces.map((p) => (
                              <option key={p.id} value={p.name}>{p.name}</option>
                            ))}
                            <option value="Étranger / Autre">Autre (Étranger)</option>
                          </select>
                        ) : (
                          <input
                            type="text"
                            id="birth-province-input"
                    ...
                    {/* Ville/Territoire de naissance */}
                    <div>
                      <label className="block text-[10px] font-bold text-slate-505 mb-0.5">Ville ou Territoire de naissance <span className="text-red-500">*</span></label>
                      <input
                        type="text"
                        id="birth-city-input"
                        placeholder="Ex: Kinshasa, Paris, Kikwit, Lubumbashi..."
                        value={birthCity}
                        onChange={(e) => setBirthCity(e.target.value)}
                        className="w-full bg-white border border-slate-200 text-slate-800 rounded-xl px-4 py-1.5 text-xs focus:border-blue-600 outline-none font-bold"
                      />
                    </div>
                  </div>
                </div>
