...
export default function ClientDashboard({
  ...
  const [activeTab, setActiveTab] = useState<"ride" | "wallet" | "profile" | "security" | "history" | "disputes" | "favorites">("ride");
  
  // Favorite Destinations & Places States
  const [favoriteLocations, setFavoriteLocations] = useState<{
    ...
    const saved = localStorage.getItem(`gomoto_favorites_${profile.id}`);
    if (saved) {
      try {
    ...
    localStorage.setItem(`gomoto_favorites_${profile.id}`, JSON.stringify(defaults));
    return defaults;
  });

  const [newFavLabel, setNewFavLabel] = useState("");
  const [newFavProvince, setNewFavProvince] = useState("kinshasa");
  const [newFavCity, setNewFavCity] = useState("Kinshasa");
  const [newFavCommune, setNewFavCommune] = useState("Gombe");
  const [newFavAvenue, setNewFavAvenue] = useState("");

  const handleNewFavProvinceChange = (provId: string) => {
    setNewFavProvince(provId);
    const prov = drcProvinces.find(p => p.id === provId);
    if (prov && prov.cities && prov.cities.length > 0) {
      setNewFavCity(prov.cities[0].name);
      if (prov.cities[0].communes && prov.cities[0].communes.length > 0) {
        setNewFavCommune(prov.cities[0].communes[0]);
      } else {
        setNewFavCommune("Centre");
      }
    }
  };

  const handleNewFavCityChange = (cityName: string) => {
    setNewFavCity(cityName);
    const prov = drcProvinces.find(p => p.id === newFavProvince) || drcProvinces[0];
    const citiesOfProv = prov.cities || [];
    const city = citiesOfProv.find(c => c.name === cityName);
    if (city && city.communes && city.communes.length > 0) {
      setNewFavCommune(city.communes[0]);
    } else {
      setNewFavCommune("Centre");
    }
  };

  const applyFavoriteAsDeparture = (fav: any) => {
    setSelectedProvinceId(fav.provinceId);
    setSelectedCityName(fav.cityName);
    setSelectedCommuneName(fav.communeName);
    setPickupRoad(fav.avenue);
    ...
    showToast(
      ...
      `Le départ a été défini sur : ${fav.label} (${fav.avenue}, ${fav.communeName})`
    );
  };

  const applyFavoriteAsDestination = (fav: any) => {
    setSelectedProvinceId(fav.provinceId);
    setSelectedCityName(fav.cityName);
    setSelectedCommuneName(fav.communeName);
    setDropoffRoad(fav.avenue);
    ...
    showToast(
      "success",
      "Destination définie",
      `La destination a été définie sur : ${fav.label} (${fav.avenue}, ${fav.communeName})`
    );
  };
  ...
  const handleAddFavorite = (e: React.FormEvent) => {
    e.preventDefault();
    if (!newFavLabel.trim()) {
      alert("Veuillez entrer une étiquette (ex: Maison).");
      return;
    }
    const avenueToSave = newFavAvenue.trim() || mockAvenues[0];
    
    const newFav = {
      id: "fav-" + Math.random().toString(36).substr(2, 6),
      label: newFavLabel.trim(),
      provinceId: newFavProvince,
      cityName: newFavCity,
      communeName: newFavCommune,
      avenue: avenueToSave
    };

    const updated = [...favoriteLocations, newFav];
    setFavoriteLocations(updated);
    localStorage.setItem(`gomoto_favorites_${profile.id}`, JSON.stringify(updated));
    ...
    showToast(
      "success",
      "Favori enregistré",
      `Le lieu "${newFav.label}" a été ajouté avec succès à votre liste d'itinéraires favoris.`
    );
  };

  const handleRemoveFavorite = (id: string, label: string) => {
    ...
    showToast(
      "info",
      "Favori retiré",
      `Le lieu "${label}" a été retiré de vos favoris.`
    );
  };
  ...
  return (
    <div id="client-screen-container" className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start max-w-7xl mx-auto relative">
        ...
        <div className="bg-white border border-slate-200 rounded-3xl p-5 shadow-sm relative overflow-hidden text-slate-800">
          ...
          <div className="bg-slate-50 border border-slate-200 rounded-3xl p-4 space-y-4 mt-5 text-left animate-fade-in shadow-sm">
            <div>
              ...
              <div className="space-y-1.5">
                ...
                >
                  <Star className={`w-4 h-4 ${activeTab === "favorites" ? "text-white text-yellow-500 fill-yellow-500" : "text-yellow-600 fill-yellow-500"}`} />
                  <span className="flex-1 text-left">Itinéraires Favoris</span>
                  <ChevronRight className="w-3.5 h-3.5 opacity-60" />
                </button>
              </div>
            </div>
          ...
          <div id="desktop-tabs-menu" className="hidden">
            ...
            <button
               ...
               onClick={() => setActiveTab("favorites")}
               className={`flex-1 text-center py-2 rounded-lg text-[10px] font-bold tracking-wider uppercase transition-all cursor-pointer ${
                 activeTab === "favorites" ? "bg-white text-slate-800 shadow-sm font-black" : "text-slate-500 hover:text-slate-800"
               }`}
            >
              Favoris
            </button>
      ...
      <div className="lg:col-span-8 space-y-6">
            ...
            {rideStatus === "idle" ? (
              <div className="bg-white border border-slate-200 rounded-3xl p-6 shadow-sm space-y-6">
                ...
                {favoriteLocations.length > 0 && (
                  <div className="bg-slate-50 border border-slate-150 p-4 rounded-2xl space-y-2.5 text-left transition-all animate-fade-in shadow-sm">
                    <div className="flex justify-between items-center">
                      <span className="text-[10px] uppercase font-bold text-slate-500 tracking-wider flex items-center gap-1">
                        <Star className="w-3 text-yellow-500 fill-yellow-500" />
                        <span>Sélection Rapide : Lieux Favoris Congolais ({favoriteLocations.length})</span>
                      </span>
                      <button
                        type="button"
                        onClick={() => setActiveTab("favorites")}
                        className="text-[9.5px] font-bold text-blue-600 hover:text-blue-800 underline cursor-pointer"
                      >
                        Gérer mes favoris
                      </button>
                    </div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {favoriteLocations.map((fav) => (
                        <div
                          key={fav.id}
                          className="bg-white border border-slate-200 hover:border-slate-300 rounded-xl p-2.5 flex items-center justify-between gap-3 text-[10.5px] shadow-sm transition-all"
                        >
                          <div className="flex flex-col">
                            <span className="font-bold text-slate-850 flex items-center gap-1">
                              {fav.label.startsWith("Maison") ? "🏠" : fav.label.startsWith("Bureau") ? "💼" : fav.label.startsWith("Chez") ? "👪" : "📍"}
                              {fav.label}
                            </span>
                            <span className="text-[8.5px] text-slate-450 mt-0.5 truncate max-w-[130px]" title={`${fav.avenue}, ${fav.communeName}`}>
                              {fav.avenue}, {fav.communeName}
                            </span>
                          </div>
                          ...
                          <div className="flex items-center gap-1.5 border-l border-slate-150 pl-2">
                            <button
                              type="button"
                              onClick={() => applyFavoriteAsDeparture(fav)}
                              className="bg-blue-50 hover:bg-blue-105 text-blue-700 font-bold px-2 py-1 rounded-md text-[9px] cursor-pointer transition-all active:scale-95 flex items-center gap-0.5 whitespace-nowrap"
                            ...
                            <button
                              type="button"
                              onClick={() => applyFavoriteAsDestination(fav)}
                              className="bg-red-550/10 hover:bg-red-550/20 text-red-700 font-bold px-2 py-1 rounded-md text-[9px] cursor-pointer transition-all active:scale-95 flex items-center gap-0.5 whitespace-nowrap"
                              title="Définir comme destination"
                            >
                              🛬 Arrivée
                            </button>
                          </div>
                ...
                {/* Instant Favorites Save buttons */}
                <div className="flex justify-between items-center text-[10px] text-slate-500 px-1 mt-0.5">
                  <button
                    type="button"
                    onClick={() => {
                      const label = prompt("Entrez un nom pour ce lieu de départ favori :", "Maison");
                      if (label) {
                        const newFav = {
                          id: "fav-" + Math.random().toString(36).substr(2, 6),
                          label: label.trim(),
                        ...
                        setFavoriteLocations(updated);
                        localStorage.setItem(`gomoto_favorites_${profile.id}`, JSON.stringify(updated));
                        showToast("success", "Départ favori enregistré", `"${label}" a été ajouté à vos favoris.`);
                      }
                    }}
                    className="text-blue-600 hover:text-blue-805 flex items-center gap-1 cursor-pointer font-bold"
                  >
                    <Star className="w-3 h-3 text-yellow-500 fill-yellow-500" />
                    <span>Sauvegarder ce départ</span>
                  </button>
                  <button
                    type="button"
                    onClick={() => {
                      const label = prompt("Entrez un nom pour ce lieu de destination favori :", "Bureau Gombe");
                      if (label) {
                        const newFav = {
                          id: "fav-" + Math.random().toString(36).substr(2, 6),
                          label: label.trim(),
                        ...
                        setFavoriteLocations(updated);
                        localStorage.setItem(`gomoto_favorites_${profile.id}`, JSON.stringify(updated));
                        showToast("success", "Destination favorite enregistrée", `"${label}" a été ajouté à vos favoris.`);
                      }
                    }}
                    className="text-blue-600 hover:text-blue-805 flex items-center gap-1 cursor-pointer font-bold"
                  >
                    <Star className="w-3 h-3 text-yellow-500 fill-yellow-500" />
                    <span>Sauvegarder cette destination</span>
                  </button>
                </div>
        ...
        {activeTab === "profile" && (
          <div className="bg-white border border-slate-200 rounded-3xl p-6 shadow-sm space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="space-y-1">
                ...
                <div className="relative">
                  <input
                    ...
                    readOnly
                    className="w-full bg-slate-50 border border-slate-200 text-slate-600 rounded-xl pl-4 pr-10 py-2.5 text-xs outline-none cursor-not-allowed select-none font-bold"
                  />
                  <Lock className="w-3.5 h-3.5 text-slate-400 absolute right-3.5 top-3.5" />
                </div>
              </div>
              ...
              <div className="space-y-1">
                <label className="text-[10px] font-bold text-slate-500 uppercase tracking-widest block">Date de Naissance (Lecture uniquement)</label>
                <div className="relative">
                  <input
                    type="text"
                    value={profile.birthDate || "01-01-1990"}
                    readOnly
                    className="w-full bg-slate-50 border border-slate-200 text-slate-600 rounded-xl pl-4 pr-10 py-2.5 text-xs outline-none cursor-not-allowed select-none font-bold font-mono"
                  />
                  <Lock className="w-3.5 h-3.5 text-slate-400 absolute right-3.5 top-3.5" />
                </div>
              </div>
              ...
              <div className="space-y-1">
                <label className="text-[10px] font-bold text-slate-500 uppercase tracking-widest block">Lieu de Naissance (Pays, Province, Ville)</label>
                <div className="relative">
                  <input
                    type="text"
                    value={`${profile.birthCity || "Kinshasa"}, ${profile.birthProvince || "Kinshasa"}, ${profile.birthCountry || "République Démocratique du Congo"}`}
                    readOnly
                    className="w-full bg-slate-50 border border-slate-200 text-slate-600 rounded-xl pl-4 pr-10 py-2.5 text-xs outline-none cursor-not-allowed select-none font-bold"
                  />
                  <Lock className="w-3.5 h-3.5 text-slate-400 absolute right-3.5 top-3.5" />
        ...
        {/* ================= TABS VIEW 7: FAVORITE PLACES MANAGEMENT ================= */}
        {activeTab === "favorites" && (
          <div className="bg-white border border-slate-200 rounded-3xl p-6 shadow-sm space-y-6 text-slate-850">
            <div className="flex justify-between items-center border-b border-slate-100 pb-4">
              <div>
                <h3 className="font-extrabold text-slate-900 text-sm flex items-center gap-2">
                  ...
                  <span>Mes Itinéraires & Lieux Favoris</span>
                </h3>
                <p className="text-[10px] text-slate-500 mt-0.5">
                  Gérer vos destinations en RDC pour un remplissage en un clic lors de vos réservations quotidiennes.
                </p>
              </div>
              <span className="bg-yellow-50 text-yellow-700 border border-yellow-200 text-[9px] font-black uppercase px-2.5 py-1 rounded-xl font-mono">
                PERSISTANCE ACTIVE
              </span>
            </div>

            {/* Main grid section: New favorite form + List of favorites */}
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 items-start">
              ...
              <div className="bg-white border border-slate-200 rounded-2xl p-5 space-y-4 shadow-sm text-left">
                <h4 className="text-xs font-bold text-slate-800 uppercase tracking-wider flex items-center gap-1.5 border-b border-slate-100 pb-2">
                  ...
                  <span>Enregistrer une nouvelle adresse</span>
                </h4>

                <form onSubmit={handleAddFavorite} className="space-y-4 text-xs">
                  {/* Label */}
                  <div>
                    <label className="block text-[10px] font-bold text-slate-500 uppercase mb-1">Étiquette du Lieu</label>
                    <input
                      type="text"
                      placeholder="Ex: Maison, Bureau, Chez Grand-Mère..."
                      value={newFavLabel}
                      onChange={(e) => setNewFavLabel(e.target.value)}
                      className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3.5 py-2 text-xs text-slate-850 outline-none focus:border-blue-600 font-bold"
                      required
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <select
                      value={newFavProvince}
                      onChange={(e) => handleNewFavProvinceChange(e.target.value)}
                      className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 text-xs text-slate-850 outline-none focus:border-blue-600 cursor-pointer font-bold"
                    >
                      {drcProvinces.map((prov) => (
                        <option key={prov.id} value={prov.id}>
                          {prov.name}
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    <div>
                      ...
                      <select
                        value={newFavCity}
                        onChange={(e) => handleNewFavCityChange(e.target.value)}
                        className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 text-xs text-slate-850 outline-none focus:border-blue-600 cursor-pointer font-bold"
                      >
                        {(drcProvinces.find(p => p.id === newFavProvince)?.cities || []).map((city) => (
                          <option key={city.name} value={city.name}>
                            {city.name}
                          </option>
                        ))}
                    ...
                    <div>
                      ...
                      <select
                        value={newFavCommune}
                        onChange={(e) => setNewFavCommune(e.target.value)}
                        className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 text-xs text-slate-850 outline-none focus:border-blue-600 cursor-pointer font-bold"
                      >
                        {((drcProvinces.find(p => p.id === newFavProvince)?.cities || []).find(c => c.name === newFavCity)?.communes || []).map((comm) => (
                          <option key={comm} value={comm}>
                            {comm}
                          </option>
                        ))}
                  ...
                  {/* Avenue Selection */}
                  <div>
                    <label className="block text-[10px] font-bold text-slate-500 uppercase mb-1">Avenue d'opération</label>
                    <select
                      value={newFavAvenue}
                      onChange={(e) => setNewFavAvenue(e.target.value)}
                      className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 text-xs text-slate-850 outline-none focus:border-blue-600 cursor-pointer text-slate-850"
                    >
                      <option value="">-- Choisir une avenue existante --</option>
                      {mockAvenues.map((av, idx) => (
                        <option key={idx} value={av} className="text-slate-800 font-sans">
                          {av}
                        </option>
                      ))}
                    </select>
                    <div className="text-[9px] text-slate-400 mt-1.5 font-bold">
                      Ou saisissez un nom d'avenue personnalisé :
                    </div>
                    <input
                      type="text"
                      placeholder="Saisir l'adresse de l'avenue..."
                      value={newFavAvenue}
                      onChange={(e) => setNewFavAvenue(e.target.value)}
                      className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3.5 py-2 text-xs text-slate-850 mt-1 outline-none focus:border-blue-600 font-bold"
                    />
                  </div>

                  <button
                    type="submit"
                    className="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-2.5 rounded-xl text-xs transition-all flex items-center justify-center gap-1.5 cursor-pointer shadow-sm"
                  >
                    <Plus className="w-4 h-4" />
                    <span>Sauvegarder ce lieu</span>
                  </button>
                </form>
              </div>
              ...
              <div className="bg-white border border-slate-200 rounded-2xl p-5 space-y-4 shadow-sm text-left">
                <h4 className="text-xs font-bold text-slate-800 uppercase tracking-wider flex items-center gap-1.5 border-b border-slate-100 pb-2">
                  ...
                  <span>Mes adresses favorites ({favoriteLocations.length})</span>
                </h4>

                {favoriteLocations.length > 0 ? (
                  <div className="space-y-3">
                    {favoriteLocations.map((fav) => (
                      <div key={fav.id} className="bg-slate-50 border border-slate-200 rounded-xl p-3.5 space-y-2 text-xs shadow-sm hover:border-slate-300 transition-all">
                        <div className="flex justify-between items-center">
                          <span className="font-bold text-slate-800 flex items-center gap-1 font-sans text-xs">
                            {fav.label.startsWith("Maison") ? "🏠" : fav.label.startsWith("Bureau") ? "💼" : fav.label.startsWith("Chez") ? "👪" : "📍"}
                            {fav.label}
                          </span>
                          <button
                            type="button"
                            onClick={() => handleRemoveFavorite(fav.id, fav.label)}
                            className="text-red-500 hover:text-red-700 text-[10.5px] font-bold cursor-pointer transition-all active:scale-95 px-1 whitespace-nowrap"
                            title="Supprimer"
                          >
                            Supprimer
                          </button>
                        ...
                        <div className="space-y-1 text-slate-700 mt-1">
                          <p className="font-bold flex items-center gap-1">
                            ...
                            <span>{fav.avenue}</span>
                          </p>
                          <p className="text-[10px] text-slate-500 mt-0.5 pl-4">
                            {fav.communeName}, {fav.cityName} (Province: {drcProvinces.find(p => p.id === fav.provinceId)?.name || fav.provinceId})
                          </p>
                        </div>

                        <div className="border-t border-slate-200 pt-2.5 flex justify-end gap-1.5 mt-2">
                          <button
                            type="button"
                            onClick={() => {
                              applyFavoriteAsDeparture(fav);
                              setActiveTab("ride");
                            }}
                            className="bg-blue-600 hover:bg-blue-700 text-white font-bold px-2.5 py-1.5 rounded-lg text-[9px] cursor-pointer transition-all active:scale-95 flex items-center gap-0.5 shrink-0"
                          >
                            🛫 Départ
                          </button>
                          <button
                            type="button"
                            onClick={() => {
                              applyFavoriteAsDestination(fav);
                              setActiveTab("ride");
                            }}
                            className="bg-red-650 hover:bg-red-700 text-white font-bold px-2.5 py-1.5 rounded-lg text-[9px] cursor-pointer transition-all active:scale-95 flex items-center gap-0.5 shrink-0"
                          >
                            🛬 Arrivée
                          </button>
                        </div>
                  ...
                  <div className="text-center py-12 px-4 space-y-2">
                    <span className="text-3xl text-slate-300 block leading-none">⭐</span>
                    <h5 className="font-bold text-xs text-slate-700">Aucun lieu favori enregistré</h5>
                    <p className="text-[10px] text-slate-500 leading-relaxed max-w-xs mx-auto">
                      Sauvegardez vos adresses fréquemment visitées à Kinshasa ou ailleurs en RDC pour commander plus rapidement.
                    </p>
                  </div>
                )}
              </div>
